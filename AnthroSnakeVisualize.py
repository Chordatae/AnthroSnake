...
def display_prevalence_below_axes(ax, malnourishment_type, prevalence, CI_tuple, loc, color=None):
    """
    Displays a malnourishment prevalence rate and its confidence interval below a set of axes. 
    Optionally, displays a colored box next to the prevalence rate.
...
def save_figures(fig1, fig2):
    """
    Saves the provided matplotlib figures to a single PDF file.

    This function prompts the user to enter a filename for the output PDF that will
    contain the visualizations. If the user presses enter without inputting a name,
    a default filename is used. The figures are then saved in the specified PDF file.
